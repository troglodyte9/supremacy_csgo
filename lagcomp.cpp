...
bool LagCompensation::StartPrediction(AimPlayer* data) {
	...
	if (!previous || previous->immune() || previous->dormant() || previous->m_flags & FL_ONGROUND)
		return false;
	...
	int lag = game::TIME_TO_TICKS(record->m_lag);
	...
	// if hes retracking / circling, delay shot
	if (fabsf(prevdir - dir) > 45.f)
		return false;
...
void LagCompensation::PredictAnimations(CCSGOPlayerAnimState* state, LagRecord* record, LagRecord* previous) {
	...
	// backup data.
	AnimBackup_t backup;
	backup.real_time = g_csgo.m_globals->m_realtime;
	backup.cur_time = g_csgo.m_globals->m_curtime;
	backup.frame_time = g_csgo.m_globals->m_frametime;
	backup.abs_frame_time = g_csgo.m_globals->m_abs_frametime;
	backup.interpolation_amount = g_csgo.m_globals->m_interp_amt;
	backup.frame_count = g_csgo.m_globals->m_frame;
	backup.tick_count = g_csgo.m_globals->m_tick_count;
	...
	g_csgo.m_globals->m_realtime = record->m_pred_time;
	g_csgo.m_globals->m_curtime = record->m_pred_time;
	g_csgo.m_globals->m_frametime = g_csgo.m_globals->m_interval;
	g_csgo.m_globals->m_abs_frametime = g_csgo.m_globals->m_interval;
	g_csgo.m_globals->m_frame = game::TIME_TO_TICKS(record->m_pred_time);
	g_csgo.m_globals->m_tick_count = game::TIME_TO_TICKS(record->m_pred_time);
	...
	g_csgo.m_globals->m_realtime = backup.real_time;
	g_csgo.m_globals->m_curtime = backup.cur_time;
	g_csgo.m_globals->m_frametime = backup.frame_time;
	g_csgo.m_globals->m_abs_frametime = backup.abs_frame_time;
	g_csgo.m_globals->m_frame = backup.frame_count;
	g_csgo.m_globals->m_tick_count = backup.tick_count;
	g_csgo.m_globals->m_interp_amt = backup.interpolation_amount;
}