...
public:
	...
	__forceinline void ModifyEyePosition(CCSGOPlayerAnimState* pState, vec3_t* pos) {
		/* NOTE FROM ALPHA:
		...
		using canimationlookupbone = int(__thiscall*)(void*, const char*);
		static auto C_BaseAnimating__LookupBone = pattern::find(g_csgo.m_client_dll, XOR("55 8B EC 53 56 8B F1 57 83 BE ?? ?? ?? ?? ?? 75 14")).as<canimationlookupbone>();

		// thanks onetap!
		auto headBone = C_BaseAnimating__LookupBone(pState->m_player, XOR("head_0"));

		vec3_t headPos;
		headPos = (reinterpret_cast<Player*>(pState->m_player))->GetBonePosition(headBone);
	...
	__forceinline vec3_t GetEyePosition() {
		vec3_t vecOrigin = this->GetAbsOrigin();

		vec3_t offset = m_vecViewOffset();
		if (offset.z >= 46.1f) {
			if (offset.z > 64.0f) {
				offset.z = 64.0f;
			}
