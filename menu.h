...
public:
	void init() {
		...
		seperate.setup(XOR("seperate point-scales"), XOR("seperate"));
		RegisterElement(&seperate);

		seperate_scale.setup("global scale", XOR("seperate_scale"), 1.f, 100.f, false, 0, 90.f, 1.f, XOR(L"%"));
		seperate_scale.AddShowCallback(callbacks::IsMultipointOn);
		seperate_scale.AddShowCallback(callbacks::IsSeperatePointscaleOff);
		RegisterElement(&seperate_scale);

		scale.setup("head scale", XOR("hitbox_scale"), 1.f, 100.f, true, 0, 90.f, 1.f, XOR(L"%"));
		scale.AddShowCallback(callbacks::IsMultipointOn);
		scale.AddShowCallback(callbacks::IsSeperatePointscaleOn);
		...
		body_scale.AddShowCallback(callbacks::IsSeperatePointscaleOn);
		RegisterElement(&body_scale);

		minimal_damage.setup(XOR("minimal damage"), XOR("minimal_damage"), 1.f, 100.f, true, 0, 40.f, 1.f, XOR(L"hp"));
		RegisterElement(&minimal_damage);

		minimal_damage_hp.setup(XOR("damage scaling"), XOR("minimal_damage_hp"));
		RegisterElement(&minimal_damage_hp);

		penetrate.setup(XOR("auto-wall"), XOR("penetrate"));
		RegisterElement(&penetrate);

		delay_unduck.setup(XOR("delay shot on unduck"), XOR("delay_unduck"));
		RegisterElement(&delay_unduck);

		hitchance_amount.setup("weapon hitchance", XOR("hitchance_amount"), 1.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
		hitchance_amount.AddShowCallback(callbacks::IsConfigMM);
		RegisterElement(&hitchance_amount,1);


		hitchance_air_amount.setup("hitchance in air", XOR("hitchance_air_amount"), 1.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
		hitchance_air_amount.AddShowCallback(callbacks::IsConfigMM);
		RegisterElement(&hitchance_air_amount, 1);

		zeusbot_hc.setup("zeus hitchance", XOR("zeusbot_hc"), 1.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
		zeusbot_hc.AddShowCallback(callbacks::IsConfigMM);
		...
		target_limit_amount.setup(XOR(""), XOR("target_limit_amount"), 3.f, 10.f, false, 0, 5.f, 1.f);
		target_limit_amount.AddShowCallback(callbacks::IsLimitingTargets);
		RegisterElement(&target_limit_amount);
		...
		override.setup(XOR("override"), XOR("override"));
...
public:
	void init() {
		...
		manual_antiaim.setup(XOR("show manual anti-aim"), XOR("manual_antiaim"));
		RegisterElement(&manual_antiaim, 1);

		color_manual_antiaim.setup(XOR("manual anti-aim color"), XOR("color_manual_antiaim"), colors::burgundy);
		RegisterElement(&color_manual_antiaim, 1);
...
public:
	void init() {
		...
		indicators.setup(XOR("indicators"), XOR("indicators"), { XOR("lby"), XOR("lag compensation"), XOR("fake latency"), XOR("override"), XOR("dmg") });
		...
		impact_beams_enemy.setup(XOR("impact beams enemy"), XOR("impact_beams_enemy"));
		RegisterElement(&impact_beams_enemy, 1);
...
public:
	void init() {
		...
		RegisterElement(&autopeek_active, 1);
