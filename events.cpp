...
void events::bullet_impact(IGameEvent* evt) {
	...
	if (g_cl.m_processing) {
		...
		if (!shooter->dormant() && shooter->IsPlayer()) {
			if (shooter->m_iTeamNum() != g_cl.m_local->m_iTeamNum() && g_menu.main.visuals.impact_beams_enemy.get()) {
				g_visuals.PushBeamInfo({ info.time, shooter->GetShootPosition(), vec3_t(info.x, info.y, info.z), g_menu.main.visuals.impact_beams_hurt_color.get(), attacker, -1 });
			}
			else if (shooter == g_cl.m_local && g_menu.main.visuals.impact_beams.get()) {
				g_visuals.PushBeamInfo({ info.time, shooter->GetShootPosition(), vec3_t(info.x, info.y, info.z), g_menu.main.visuals.impact_beams_color.get(), attacker, -1 });
			}
		}
...
void events::player_death( IGameEvent* evt ) {
	...
	auto attacker = g_csgo.m_engine->GetPlayerForUserID(evt->m_keys->FindKey(HASH("attacker"))->GetInt());
	auto local_player = g_csgo.m_engine->GetLocalPlayer();
	auto is_headshot = evt->GetBool("headshot");

	if ((attacker == local_player && index != local_player) && is_headshot) {
		taps++;
	}
