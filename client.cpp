...
void Client::DrawHUD( ) {
	...
	std::string text = tfm::format( XOR( "lol | rtt: %i | taps: %i"), rate, taps);
...
void Client::SetAngles( ) {
	...
	// set radar angles.
	g_csgo.m_prediction->SetLocalViewAngles( g_cl.m_real_angle );
}
...
void Client::UpdateLocal( )
{
	...
	// backup curtime, m_fEffects and frametime.
	...
	// llama does it.
	state->m_abs_yaw_last = 0.f;

	// CCSGOPlayerAnimState::Update, bypass already animated checks.
	if( state->m_frame >= tickbase_interval )
		state->m_frame = tickbase_interval - 1;

	// fix landing anim.
	if( state->m_landing && state->m_anim_duck_amount > 0.f )
		m_real_angle.x = -12.f;

	// update anim update delta like the server does.
	state->m_last_update_increment = std::max( 0.0f, g_csgo.m_globals->m_curtime - state->m_last_update_time );

	// is it time to update?
	if( g_csgo.m_globals->m_curtime != state->m_last_update_time ) {
		// allow the game to update animations this tick.
		g_cl.m_update_anims = true;

		// update our animation state.
		game::UpdateAnimationState( state, ang_t( m_real_angle.x, m_real_angle.y, m_real_angle.z ) );
	...
	if( g_cl.m_layers ) {
		...
		g_cl.m_layers[12].m_weight = 0.0f;
	}
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf( colors::burgundy, XOR( "[lol] " ) );
